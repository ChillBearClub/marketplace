async function displayErrorMessage(message, timed = true) {
  if (!$("#error-popup").length) {
    ...
    $("body").append(
      `<div id='block-screen-error' style="height:${height}px"></div>`
    );
    if (timed) {
      await sleep(2500);
      $("#error-popup").remove();
...
const loadInfuraListings = async () => {
  ...
  if (jsonData.numLive == 0) {
    $("#live-collections").append(
      "<div id='no-live-msg'><h2>No active listings.<br>Join our discord to see what's next!</h2><br><a href='https://discord.com/invite/anonymice' target='_blank'><button class='button'>JOIN DISCORD</button></a></div>"
    );
  }
};
...
const identityMapper = new ethers.Contract(
  identityMapperAddress,
  identityMapperAbi(),
  signerID
);
...
const promptForDiscord = async () => {
  if (!$("#discord-popup").length) {
    ...
    let height = $(document).height();
    $("body").append(
      `<div id='block-screen-discord' style="height:${height}px" onclick="$('#discord-popup').remove();$('#block-screen-discord').remove()"></div>`
    );
  }
};
