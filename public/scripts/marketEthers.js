...
}; // divides by 18 modulus
const parseEther = (eth_) => {
  return ethers.utils.parseEther(eth_);
}; // multiplies by 18 modulus
...
const loadingDiv = `<div id="ex1" class="partner-collection example">
                        ...
                        <div class="collection-info">
                            ...
                            <span style="display: flex;
                                    ...
                                    align-items: center;
                                    gap: 11px;">??? <img src="${tokenImgURL}" class="token-icon"></span>
                            <br>
                            ...
                            <button class="button"><i class="fa-solid fa-cart-shopping"></i> PURCHASE</button>
                        </div>
                    </div>`;
...
const approveTokenToMarket = async () => {
  await cheeth.approve(marketAddress, maxInt).then(async (tx_) => {
    ...
    $("#approval-button").html(
      `Approving<span class="one">.</span><span class="two">.</span><span class="three">.</span>`
    );
  });
};
...
const updateTokenBalance = async () => {
  const userAddress = await getAddress();
  let balance = Number(
    formatEther(await cheeth.balanceOf(userAddress))
  ).toFixed(1);
  ...
  try {
    if (!discordSet) {
      await displayErrorMessage(
        "Error: Must set Discord ID to associate with purchases!"
      );
    ...
    if (error.message.includes("Already purchased")) {
      ...
      await displayErrorMessage(`Error: Not enough $STAR!`);
    } else if (error.message.includes("transfer amount exceeds allowance")) {
      await displayErrorMessage(`Error: Market not approved to spend $STAR!`);
  ...
  if (loadedCollections) {
    for (let i = 0; i < liveListings.length; i++) {
      if (liveTimerPending[i]) {
        ...
        let endTime = Number(
          (await market.contractToWLVendingItems(cheethAddress, id)).endTime
        );
        ...
        if (distance <= 0) {
          ...
          if (blockTime > endTime) {
            ...
            $(`#timer-${id}`).html(
              `ENDS NEXT BLOCK<span class="one">.</span><span class="two">.</span><span class="three">.</span>`
            );
          }
        } else {
    ...
    for (let i = 0; i < pendingListings.length; i++) {
      if (pendingTimerPending[i]) {
        ...
        let startTime = Number(
          (await market.contractToWLVendingItems(cheethAddress, id)).startTime
        );
        ...
        if (distance <= 0) {
          ...
          if (blockTime > startTime) {
            ...
            $(`#timer-${id}`).html(
              `LIVE NEXT BLOCK<span class="one">.</span><span class="two">.</span><span class="three">.</span>`
            );
          }
        } else {
...
const loadCollections = async () => {
  ...
  const numCollections = Number(
    await market.getWLVendingItemsLength(cheethAddress)
  );
  let allItems;
  if (numCollections > 0) {
    allItems = await market.getWLVendingObjectsPaginated(
      cheethAddress,
      0,
      numCollections - 1
    );
  ...
  for (const chunk of chunks) {
    await Promise.all(
      chunk.map(async (id) => {
        ...
        let purchased = await market.contractToWLPurchased(
          cheethAddress,
          id,
          userAddress
        );
        ...
        let imageUri = WLinfo.imageUri.includes("https://")
          ? WLinfo.imageUri
          : `https://${WLinfo.imageUri}`;
        let projectUri = WLinfo.projectUri.includes("https://")
          ? WLinfo.projectUri
          : `https://${WLinfo.projectUri}`;

        if (started && valid) {
          liveListings.push(id);
          liveTimerPending.push(true);
          dateString = `Ends ${new Date(
            WLinfo.endTime * 1000
          ...
          dateString = `Starts ${new Date(
            WLinfo.startTime * 1000
          ).toLocaleDateString()} ${new Date(
            WLinfo.startTime * 1000
          ).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
        }
        ...
        if (minted != maxSlots && valid) {
          ...
          if (purchased) {
            ...
            button = `<button class="mint-prompt-button button" id="${id}-mint-button" onclick="purchase('${cheethAddress}', ${id})"><i class="fa-solid fa-cart-shopping"></i> PURCHASE</button>`;
          }
  ...
  let liveIds = Array.from(idToLiveJSX.keys())
    .map(Number)
    .sort(function (a, b) {
      return b - a;
    });
  let pastIds = Array.from(idToPastJSX.keys())
    .map(Number)
    .sort(function (a, b) {
      return b - a;
    });
